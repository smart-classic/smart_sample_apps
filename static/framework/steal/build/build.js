steal(function(steal){
	...
	steal.build = function(url, options){
		...
		print("Building to "+options.to+", opening " + url);
	...
	steal.build.open = function(url ){
		...
		print ("firing envjs");
		Envjs(url, {scriptTypes: types, fireLoad: false, logLevel: 4,
	        afterScriptLoad: {".*": function(script){ 
			print ("after on " + script.src);
	                scripts.push(script);
			print ($('html').html());
	            }
	        },
	        onLoadUnknownTypeScript: function(script){
		print ("unktyo " + script.src);
	            self.scripts.push(script);   
	        },
			afterInlineScriptLoad : function(script){
		print ("donelinline " + script.src);
				scripts.push(script);   
	        }
	    }); 
		//set back steal
